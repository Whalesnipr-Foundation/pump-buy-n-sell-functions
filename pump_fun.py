...
def buy(mint_str: str, sol_in: float = 0.01, slippage: int = 25, priority_in_lamports: int = 5_000_000 ) -> bool:
    try:
        # Get coin data
        ...
        print(f"Transaction confirmed? {confirm}")
        return confirm
        
    except Exception as e:
        print(e)

def sell(mint_str: str, priority_in_lamports : int ,token_balance: Optional[Union[int, float]] = None,  slippage: int = 25, close_token_account: bool = True) -> bool:
    try:
        # Get coin data
        coin_data = get_coin_data(mint_str)
        print('Sell coin data')
        print(coin_data)
        if not coin_data:
            print("Failed to retrieve coin data...")
            return
        ...
        #Wait 5 seconds before attempt to get token balance
        # time.sleep(5)
        ...
        print("Transaction Signature", txn_sig)
        confirm = confirm_txn(txn_sig)
        print(f"Sell confrimed ? {confirm}")
        return confirm 
